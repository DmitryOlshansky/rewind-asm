module rewind.dynasm.arm64;

import rewind.dynasm.mem;
...
struct Assembler {
    ...
    this(size_t sizeInBytes) {
        buffer = cast(uint[])allocateVM(sizeInBytes);
    }
    ...
    void finalize() {
        makeExecutableAndFlushCache(buffer);
    }
    ...
    private void emit(uint instr) {
        if (count == buffer.length) {
            buffer = cast(uint[])reallocateVM(buffer, buffer.length * 2);
        }
        buffer[count++] = instr;
    }
...
unittest {
    auto assembler = Assembler(pageSize);
    with(assembler) {
